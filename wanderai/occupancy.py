...
class OccupancyGrid:
    ...
    def from_scene(cls, scene: Scene, cell_size: float = 0.1) -> "OccupancyGrid":
        ...
        b = scene.bounds
        ncols = int(math.ceil((b.max_x - b.min_x) / cell_size))
        nrows = int(math.ceil((b.max_y - b.min_y) / cell_size))
        ...
        return cls(blocked, cell_size, (b.min_x, b.min_y), b)

    def world_to_cell(self, x: float, y: float) -> tuple[int, int]:
        c = math.floor((x - self.origin[0]) / self.cell_size)
        r = math.floor((y - self.origin[1]) / self.cell_size)
        return r, c
    ...
    def is_blocked_world(self, x: float, y: float) -> bool:
        if self.bounds is not None and not self.bounds.contains(x, y):
            return True
